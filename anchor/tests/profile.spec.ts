...
console.log('IDL instructions:', profileIdl.instructions?.map(i => i.name));
...
describe('profile program', () => {
  ...
  it('can create / set a profile (init_if_needed)', () => {
    ...
    console.log('PDA derivation correct:', expectedPda.toString() === profilePda.toString());
    ...
    // Decode and verify
    const profile = program.coder.accounts.decode('profile', Buffer.from(accountInfo.data));
